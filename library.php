...
class Bitcoinnova_Library {
    ...
    public function __construct($pUser, $pPass, $pHost, $pPort) {
        ...
        $this->pass = $pPass;
        $this->user = $pUser;
        $this->url = $pUser.':'.$pPass.'@'. $pHost . ':' . $pPort.'/';
    }
    ...
    protected function getResponse($pRequest) {
        ...
        if (0 < curl_errno($ch)) {
           echo '[ERROR] Failed to connect to zenzo-wallet-rpc at user ' . $this->user. ' pass '. $this->pass. ' host ' . $this->host . ' port '. $this->port .'</br>';
        }
    ...
    public function getStatus() {
        $status = $this->_run('getbestblockhash', null);
        return $status;
    }
    ...
    public function getPayments($lastBlockHash, $paymentId) {
        ...
        $get_payments = $this->_run('gettransaction', $payment_param);
        return $get_payments;
    }
}
